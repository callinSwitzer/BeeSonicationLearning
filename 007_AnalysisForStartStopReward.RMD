...
packages <- c("ggplot2", "reshape2", 'lme4', 'sjPlot', "multcomp", "plyr", "effects", "viridis", "GGally", "survminer")
ipak(packages)
...
ggplot(bees, aes(x = rewNum, y = freq, color = treatment_rewarded)) +
...
survBees <- t(sapply(unique(bees$beeID), function(x) {
     ...
     return(tmp[which.max(tmp$rewNum), ])
}))


lastObs <- as.data.frame(survBees)
lastObs$index <- as.numeric(lastObs$rewNum)
