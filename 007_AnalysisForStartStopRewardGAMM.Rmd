...
freqPlot <- ggplot(pframe[pframe$hive == 7 & pframe$IT_mm == ITmean & !(duplicated(pframe[, c("blo", "bhi", "predMean")])), ], aes(x=treatment_rewarded2, y=predMean))+
     ...
     geom_errorbar(aes(ymin = blo, ymax = bhi), width = 0.1) + 
  annotate(geom="text", x=c(1,2), y=c(0, 0) + 345, label=c("a", "b"),
                color="black")
freqPlot
ggsave(plot = freqPlot, filename = file.path(figDir, "StartStop_Freq.pdf"), width =2.5, height = 3)
...
# make centered model, to make it a bit more interpretable
# for IT span
finmod_centered <- update(finmod, .~. - IT_mm*treatment_rewarded  + I(IT_mm - ITmean)*treatment_rewarded)
...
freqPlotIT
ggsave(plot = freqPlotIT, filename = file.path(figDir, "StartStopBootFreq_ITHivePlot_SUPP2.pdf"), width =5, height = 3.7)
