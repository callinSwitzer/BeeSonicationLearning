...
timeDF <- as.tbl(bees) %>%
  dplyr::select(BeeNumCol, treatment, treatment_rewarded, visitNum) %>%
...
timeDF2 = timeDF %>%
  ...
  mutate(death_ = (censor2 == "give up")*1, 
...
# cox model
m1 <- coxph(formula = Surv(time1, time2, death_) ~  pollenR2, data = timeDF2)
summary(m1)

m2 <- coxph(formula = Surv(time1, time2, death_) ~  treatment + pollenR2, data = timeDF2)
...
km.by.trt <- survfit(Surv(time1, time2, death_) ~ treatment, data = timeDF2, conf.type = "log-log")
...
s4 <- coxph(survObj ~ Treatment + unlist(IT_mm), data = lastObs) 
...
# refref: censor not right
# refref: plot not right.
...
ggsurv(km.by.trt, CI = FALSE, surv.col = c("#a6cee3", "#1f78b4"), plot.cens = FALSE) + 
     ylim(c(0, 1)) + 
     xlab("Sonication Number") + 
     ylab("Cumulative Proportion Sonicating") + 
...
ggsave(file.path(figDir, "beeSonicationPercentage_onePlot.svg"), width = 5, height = 3.5)
