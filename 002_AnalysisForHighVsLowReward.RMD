...
# make a different version of the plot
```{r}
...
ggsave(plot = g1, filename = file.path(figDir, "SonicationFreqPredsAndCI_unadjusted_overlap.png"), unit = "in", dpi = 500,
       width = 6.5, height = 5)
ggsave(plot = g1, filename = file.path(figDir, "SonicationFreqPredsAndCI_unadjusted_overlap.svg"), width = 6.5, height = 5)
...
# don't need hive, because that's not in the model we chose (above)
pframe <- data.frame(expand.grid(trt = levels(sl$trt), 
                                 IT_imputed = seq(min(s2$IT_imputed), max(s2$IT_imputed), length.out = 5),  
                                 beeColHive  = 99999, 
                                 trialNum = 2))
pframe$freq <- 0
pp <- predict(m3, newdata = pframe, re.form=NA, type = 'response') # re.form sets all random effects to 0
...
g2 <- ggplot(pframe, aes(x=IT_imputed, y=predMean, color = trt3, fill = trt3))+
        geom_point(position = position_dodge(0.1), size = 2)+
        geom_line(position = position_dodge(0.1), size = 1, alpha = 0.5)+
        labs(y = "Sonication frequency (Hz)", x = "Intertegular span (mm)") + 
        geom_errorbar(data = pframe, aes(x = IT_imputed, ymin = blo,
                                      ymax = bhi, color = trt3), 
                                      position = position_dodge(0.1), lwd = 1.2,
                      inherit.aes = FALSE, width = 0.05) +
        theme(plot.background = element_rect(fill = "transparent",colour = NA), 
            panel.background = element_rect(fill = "transparent",colour = NA), 
            legend.position =c(0.8, 0.8), 
            strip.background = element_blank(), 
...
ggsave(plot = g2, filename = file.path(figDir, "SonicationFreqPredsAndCI_unadjusted_points.png"), unit = "in", dpi = 500,
       width = 6.5, height = 5)
ggsave(plot = g2, filename = file.path(figDir, "SonicationFreqPredsAndCI_unadjusted_points.svg"), width = 6.5, height = 5)
