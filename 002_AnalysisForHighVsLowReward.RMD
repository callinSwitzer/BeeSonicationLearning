...
sl_sum <- sl %>%
  dplyr::select(beeCol, trt2, IT_imputed) %>%
  distinct(beeCol, trt2, .keep_all = TRUE) 
...
set.seed(123)
ggplot(s2, aes(x = trt3,  y= IT_imputed)) + 
...
# library(plotly)
# 
# p <- plot_ly(sl[sl$trt2 == "full", ], x = ~freq, y = ~IT_imputed, z = ~amp_acc2,
#         marker = list(color = ~amp_acc2, colorscale = c('#FFE1A1', '#683531'), showscale = TRUE)) %>%
#   add_markers() %>%
#   layout(scene = list(xaxis = list(title = 'freq'),
#                      yaxis = list(title = 'size'),
#                      zaxis = list(title = 'accel')),
#          annotations = list(
#            x = 1.13,
#            y = 1.05,
#            text = 'accel',
...
m2 = lmer(freq ~ trt2 + hive + trialNum0 + IT_imputed + (1+trialNum0|beeColHive), data = sl, REML = FALSE)
summary(m2)

m2 = lmer(freq ~ trt2 + hive  + IT_imputed + trialNum0 + initialFreq*trt2 +   (1|beeColHive), data = initialFreqAmp, REML = FALSE)
summary(m2)
...
m2_1 = lmer(freq ~ trt2* IT_imputed + hive + trialNum0*trt2  + (1+trialNum|beeColHive), data = sl, REML = FALSE)

m2_1 = lmer(freq ~ trt2 * IT_imputed + hive + (1|beeColHive), data = initialFreqAmp, REML = FALSE)
