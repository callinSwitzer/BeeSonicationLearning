...
ggplot(fta, aes(x = trialNum, y = freq, color = t5)) + 
  ...
  geom_ribbon(data = fta[fta$trialNum > 1,], aes(x = rep(seq(1.5, 12, 3),  length.out = nrow(fta[fta$trialNum > 1,])), ymin = lowFrq, ymax = highFrq), alpha = 0.2, color = NA) + 
  geom_ribbon(data = fta[fta$trialNum > 1,], aes(x = rep(seq(0, 1.5, 1.5),  length.out = nrow(fta[fta$trialNum > 1,])), ymin = 220, ymax = 450), alpha = 0.2, color = NA) + 
  stat_smooth(data = fta[fta$trialNum > 1,], method = 'loess', se = FALSE, size =1.5, color = 'black', span = 0.5) + 
  stat_smooth(data = fta[fta$trialNum > 1,], method = 'loess', se = FALSE, size =1, span = 0.5) + 
  
  geom_hline(aes(yintercept = initialFreq)) + 
  #geom_boxplot(aes(group = trialNum), outlier.colour = NA) + 
  ...
  labs(y = "Frequency change (Hz)", x = "Trial Number")
